...
# =========================================================
# 시스템에서 반환되는 메세지 개수 출력하는 함수
# 최대 메세지 수(현재 20개로 설정) 최신 메세지 수 개수중 작은 값 리턴
# =========================================================
...
def render_login() -> None:
    ...
    # Supabase에서 이메일 확인 기능이 켜져 있을 때 발생할 수 있음.
...
def render_sidebar(
    ...
    with st.sidebar:
        ...
        if conversations:
            ...
            # 화면을 다시 그릴 때 선택이 풀릴 수 있다.

            # 선택 위젯을 그리고
            # 사용자가 특정 대화를 선택하면 selected에 저장한다.

            selected = st.selectbox(
                "이전 면접 연습 대화 내역입니다.",
                options=ids,

                # DB의 id 대신 보기 좋은 제목 출력
                format_func=lambda cid: labels[cid],

                # 기존에 선택한 대화가 있으면 그것을 유지
                index=(
                    ids.index(current)
                    if current in ids
                    else 0
        ...
        if (
            ...
            # 아래처럼 selectbox 상태를 직접 변경할 필요 없음
...
def render_follow_ups() -> None:
    ...
    주의:
    오늘은 모델이 이전 대화를 기억하지 못한다.
    (19일차 주제)

    그래서 직전 답변을 질문 안에 넣어서 보낸다.
    맥락은 결국 프롬프트로 들어간다.
    """
...
def render_conversation(
    ...
    for message in messages:
        ...
        #with : 자체는 특정 작업을 하는 동안 사용할 "문맥(context)"을 열어주는 문법
        #with 컨테이너:화면에_그릴것()
    ...
    # 메시지 목록이 있고,
    # 목록의 마지막 메시지 role이 assistant일 때만 호출 가능!!

    # 제일 마지막에 있는 메시지를 꺼내서 확인한다.
    ...
    #4_대화 맥락을 활용한 연속 대화 UX.md -> 실습7
    #메세지를 기억하는 코드로 바꿨기 때문에 마지막 메세지 넘겨주던 것 삭제
    ...
    if answer := st.chat_input(
        ...
        # 이제는 ask()에서
        # 사용자 질문 저장 + AI 호출 + AI 답변 저장을
        # 백엔드에서 처리하기 때문에 아래 코드는 필요 없다.
        ...
        # ask() 함수 안에서 이미 st.rerun()을 실행한다.
        # 따라서 여기에서 다시 st.rerun()할 필요 없음.
...
def render_signed_in() -> None:
    ...
    # 평소에는 거의 닿지 않는다.
    #
    # 하지만 목록이 있는데 선택값이 비어 있으면
    #
    # render_conversation(None)
    #
    # 이 되어 백엔드에서 422가 날 수 있다.
